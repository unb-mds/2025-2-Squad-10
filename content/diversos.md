---
title: "Diversos"
subtitulo: "Conteudos Diversos"
draft: false
layout: "embed"  # <-- ESSA É A MÁGICA!

blocos_de_texto:
  - titulo: "Story Map"
    texto: |
        <iframe 
            style="border: none; width: 100%; height: 50vh;" 
            src="https://www.figma.com/embed?embed_host=share&url=https%3A%2F%2Fwww.figma.com%2Fboard%2F8Jsltq8BOL65CsMoRWFjik%2FTemplate-MDS--Copy-%3Fnode-id%3D0-1%26p%3Df%26t%3DfTWSV1MApYy1OUtE-0" 
            allowfullscreen>
        </iframe>

        [Link para o protótipo](https://www.figma.com/embed?embed_host=share&url=https%3A%2F%2Fwww.figma.com%2Fboard%2F8Jsltq8BOL65CsMoRWFjik%2FTemplate-MDS--Copy-%3Fnode-id%3D0-1%26p%3Df%26t%3DfTWSV1MApYy1OUtE-0)

  - titulo: "Protótipo de Alta Fidelidade"
    texto: |
        <iframe 
            style="border: none; width: 100%; height: 50vh;" 
            src="https://www.figma.com/design/XyUsffocEKRw7przVsbk0n/alta-fidelidade?node-id=0-1&p=f&t=e1W7qmU51iHJ3Ahq-0" 
            allowfullscreen>
        </iframe>

        [Link para o protótipo](https://www.figma.com/design/XyUsffocEKRw7przVsbk0n/alta-fidelidade?node-id=0-1&p=f&t=e1W7qmU51iHJ3Ahq-0)
    
  - titulo: "Protótipo de Baixa Fidelidade"
    texto: |
        <iframe 
            style="border: none; width: 100%; height: 50vh;" 
            src="https://www.figma.com/design/td5oKsmfHCtT9CSPFzKU13/baixa-fidelidada?node-id=0-1&p=f&t=gQkvrPDvmG8fQfLl-0" 
            allowfullscreen>
        </iframe>

        [Link para o protótipo](https://www.figma.com/design/td5oKsmfHCtT9CSPFzKU13/baixa-fidelidada?node-id=0-1&p=f&t=gQkvrPDvmG8fQfLl-0)

  - titulo: "Quem Somos"
    texto: |
        O **OncoMap** é um projeto de *software* desenvolvido por uma equipe dedicada de alunos do curso de **Engenharia de Software** da **Universidade de Brasília (UnB)**, como parte da disciplina de Métodos de Desenvolvimento de Software (MDS) no semestre 2025.2.
        Nosso time colaborativo foi responsável por todo o ciclo de vida do projeto, desde o planejamento ágil até a implementação e os testes.
        <br>
        ### 👥 A Equipe
        <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 50px;">
            <div style="text-align: center; width: 200px; margin-bottom: 0px;">
                <a href="https://github.com/Flyxs" title="Felype Carrijo no GitHub" target="_blank">
                    <img src="https://avatars.githubusercontent.com/u/168106790?v=4" alt="Foto de Felype Carrijo" style="width: 100%; height: auto; border-radius: 50%;">
                </a>
                <p><strong>Felype Carrijo</strong><br>
                <em>Frontend</em><br>
                <a href="https://github.com/Flyxs" target="_blank">GitHub</a></p>
            </div>
            <div style="text-align: center; width: 200px; margin-bottom: 0px;">
                <a href="https://github.com/Gotc2607" title="Giovani Coelho no GitHub" target="_blank">
                    <img src="https://avatars.githubusercontent.com/u/176083022?v=4" alt="Foto de Giovani Coelho" style="width: 100%; height: auto; border-radius: 50%;">
                </a>
                <p><strong>Giovani Coelho</strong><br>
                <em>Frontend</em><br>
                <a href="https://github.com/Gotc2607" target="_blank">GitHub</a></p>
            </div>
            <div style="text-align: center; width: 200px; margin-bottom: 0px;">
                <a href="https://github.com/Luizz97" title="Luiz no GitHub" target="_blank">
                    <img src="https://avatars.githubusercontent.com/u/212640680?v=4" alt="Foto de Luiz" style="width: 100%; height: auto; border-radius: 50%;">
                </a>
                <p><strong>Luiz</strong><br>
                <em>Frontend</em><br>
                <a href="https://github.com/Luizz97" target="_blank">GitHub</a></p>
            </div>
            <div style="text-align: center; width: 200px; margin-bottom: 0px;">
                <a href="https://github.com/ArturFGaldino" title="Artur Galdino no GitHub" target="_blank">
                    <img src="https://avatars.githubusercontent.com/u/187340217?v=4" alt="Foto de Artur Galdino" style="width: 100%; height: auto; border-radius: 50%;">
                </a>
                <p><strong>Artur Galdino</strong><br>
                <em>Backend</em><br>
                <a href="https://github.com/ArturFGaldino" target="_blank">GitHub</a></p>
            </div>
            <div style="text-align: center; width: 200px; margin-bottom: 0px;">
                <a href="https://github.com/joaoPedro-201" title="João Pedro no GitHub" target="_blank">
                    <img src="https://avatars.githubusercontent.com/u/178330046?v=4" alt="Foto de João Pedro" style="width: 100%; height: auto; border-radius: 50%;">
                </a>
                <p><strong>João Pedro</strong><br>
                <em>Backend</em><br>
                <a href="https://github.com/joaoPedro-201" target="_blank">GitHub</a></p>
            </div>
            <div style="text-align: center; width: 200px; margin-bottom: 0px;">
                <a href="https://github.com/Alexandroni07" title="Gabriel Alexandroni no GitHub" target="_blank">
                    <img src="https://avatars.githubusercontent.com/u/170197026?v=4" alt="Foto de Gabriel Alexandroni" style="width: 100%; height: auto; border-radius: 50%;">
                </a>
                <p><strong>Gabriel Alexandroni</strong><br>
                <em>Backend</em><br>
                <a href="https://github.com/Alexandroni07" target="_blank">GitHub</a></p>
            </div>
        </div>

---
